...
渲染器是 Angular 为我们提供的一种内置服务，用于执行 UI 渲染操作。在浏览器中，渲染是将模型映射到视图的过程。模型的值可以是 JavaScript 中的原始数据类型、对象、数组或其它的数据对象。然而视图可以是页面中的段落、表单、按钮等其他元素，这些页面元素内部使用 DOM 来表示。
...
- 针对 button 我们只需修改其样式，因此在这里创建属性指令
- 提供属性接口
- 根据其传入的属性值动态渲染 DOM

至此，最简单的 button 就开发结束。
